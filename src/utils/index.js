export default {
   ...
   */
  openWindow: (url, targetType = '_blank', id = 'open', download = false, name) => {
    // 如果存在则删除
  ...
  mergeObjs: (...arr) => {
    ...
    arr.forEach(item => {
      for (const key in item) {
        ...
        if (type === '[object Object]') {
          obj[key] = obj[key] || {}
          mergeObj(obj[key], item[key])
        } else if (type === '[object Array]') {
          obj[key] = obj[key] || []
          mergeArray(obj[key], item[key])
        } else {
          obj[key] = item[key]
        }
      }
    ...
    function mergeObj (obj1, obj2) {
      for (const key in obj2) {
        const type = Object.prototype.toString.call(obj2[key])
        if (type === '[object Object]') {
          obj1[key] = obj1[key] || {}
          mergeObj(obj1[key], obj2[key])
        } else if (type === '[object Array]') {
          obj1[key] = obj1[key] || []
          mergeArray(obj1[key], obj2[key])
        } else {
          obj1[key] = obj2[key]
        }
      }
    ...
    // 递归数组
    function mergeArray (obj1, arr2) {
      arr2.forEach((item, index) => {
        const type = Object.prototype.toString.call(item)
        if (type === '[object Object]') {
          obj1[index] = obj1[index] || {}
          mergeObj(obj1[index], arr2[index])
        } else if (type === '[object Array]') {
          obj1[index] = obj1[index] || []
          mergeArray(obj1[index], arr2[index])
        } else {
          obj1[index] = arr2[index]
        }
      })
    }
    return obj
